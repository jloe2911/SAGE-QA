...
DATASETS = (
    ("HotpotQA", "hotpotqa", "text", "data/raw/hotpot_qa/distractor/validation-00000-of-00001.parquet"),
    ("2WikiMultiHopQA", "2wiki", "text", "data/raw/2WikiMultihopQA/data/validation-00000-of-00001.parquet"),
...
PROHIBITED_TEST_FIELDS = {
    "answer", "answers", "evidences", "supporting_facts", "raw_supporting_facts",
    "gold_explanations", "gold_units", "gold_support_units", "label", "rank_target",
...
def grouped_clean_rows(path: Path) -> Iterable[tuple[str, list[dict[str, Any]]]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if forbidden:
                raise ValueError(f"TEST candidate row contains prohibited fields: {sorted(forbidden)}")
...
def text_native_sample(rows: list[dict[str, Any]], max_candidates: int) -> tuple[dict[str, Any], dict[str, str]]:
    seed = rows[0]
    ...
    for unit in candidate_units:
        ...
        tuples.extend(([q_node, "mentions", title_node], [title_node, f"sentence_{idx}", ev_node], [q_node, "retrieved_evidence", ev_node]))
    ...
    for unit in kg_units:
        ...
        tuples.extend(([subj_node, relation, obj_node], [q_node, "kg_context", subj_node], [q_node, "kg_context", obj_node]))
...
def ontology_native_sample(rows: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if not q_entities:
        q_entities = [unit_node(candidate_units[0])] if candidate_units else [f"QUESTION::{hashlib.sha256(example_id.encode()).hexdigest()[:24]}"]
...
def prepare_clean_native_test(
    test_path: Path, native_dir: Path, source_native_dir: Path, domain: str, text_max_candidates: int
    ...
    with (native_dir / "test.json").open("x", encoding="utf-8", newline="\n") as handle:
        for example_index, (_, rows) in enumerate(grouped_clean_rows(test_path), start=1):
            ...
            if example_index % 100 == 0:
                print(f"GNN-RAG adapter {test_path.parent.name}: froze {example_index} examples", flush=True)
    if missing_relations:
        raise ValueError(f"Clean TEST uses relations absent from frozen GNN-RAG vocabulary: {sorted(missing_relations)[:10]}")
...
def run_native_retriever(
    ...
    command = [
        ...
        "--entity_dim", "50", "--data_folder", str(native_dir.resolve()) + "\\",
        "--lm", "lstm", "--num_iter", "2", "--num_ins", "2", "--num_gnn", "3",
        "--relation_word_emb", "false", "--checkpoint_dir", str(checkpoint_dir.resolve()),
        "--experiment_name", experiment, "--load_experiment", checkpoint_name,
        "--is_eval", "--name", f"sageqa-{adapter_key}",
    ]
...
def native_paths(sample: Mapping[str, Any], candidates: Sequence[Sequence[Any]]) -> tuple[list[list[list[str]]], list[str]]:
    graph = nx.Graph()
        ...
        for tail in candidate_entities:
            ...
            try:
                for nodes in nx.all_shortest_paths(graph, head, tail):
                    result.append([
                        (nodes[index], graph[nodes[index]][nodes[index + 1]]["relation"], nodes[index + 1])
                        for index in range(len(nodes) - 1)
                    ])
            except nx.NetworkXException:
                pass
    return [[list(edge) for edge in path] for path in result], [path_string(path) for path in result]


def freeze_native_output(
    dataset: str, samples: Sequence[Mapping[str, Any]], unit_maps: Sequence[Mapping[str, str]], info_path: Path
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    info_rows = [json.loads(line) for line in info_path.read_text(encoding="utf-8").splitlines() if line.strip()]
    if len(info_rows) != len(samples):
        raise ValueError(f"GNN-RAG output count mismatch for {dataset}: {len(info_rows)} != {len(samples)}")
    records = []
    for sample, unit_map, info in zip(samples, unit_maps, info_rows):
        ...
        records.append({
            ...
            "native_ranked_entities": [{"entity": item[0], "probability": item[1]} for item in candidates],
    ...
    diagnostics = {
        ...
        "examples_with_ranked_entities": sum(bool(row["native_ranked_entities"]) for row in records),
        "examples_with_nonempty_path_context": sum(bool(row["retrieved_context"]) for row in records),
        "examples_with_mapped_evidence_units": sum(bool(row["retrieved_evidence_units"]) for row in records),
        "mean_ranked_entities": mean([len(row["native_ranked_entities"]) for row in records]),
        "median_ranked_entities": statistics.median([len(row["native_ranked_entities"]) for row in records]) if records else 0.0,
        "mean_retrieved_paths": mean([len(row["retrieved_paths"]) for row in records]),
        "mean_mapped_evidence_units": mean([len(row["retrieved_evidence_units"]) for row in records]),
...
def evaluate_lexical(
    ...
    for record in records:
        ...
        for setting in ("k1", "k3"):
            scores = best_scores(record["settings"][setting]["retrieved_evidence_units"], references) if eligible else {"precision": None, "recall": None, "f1": None}
            record["settings"][setting].update(scores)
            if eligible:
                by_setting[setting].append(scores)
    metrics = {}
    for setting, rows in by_setting.items():
        metrics[setting] = {metric: statistics.fmean(row[metric] for row in rows) for metric in ("precision", "recall", "f1")}
    return {"prediction_examples": len(records), "evaluation_examples": len(records) - excluded, "excluded_no_gold_support": excluded, "settings": metrics}, records
...
def lexical_summary(metrics: Mapping[str, Any]) -> str:
    ...
    for row in metrics["datasets"]:
        for setting in ("k1", "k3"):
            ...
            lines.append(f"| {row['dataset']} | {setting} | {row['prediction_examples']} | {row['evaluation_examples']} | {score['precision']:.6f} | {score['recall']:.6f} | {score['f1']:.6f} |")
    lines.extend(["", "No adaptive-k policy was introduced. Exact retrieved supports are in `per_example_retrieval.jsonl`.", ""])
    return "\n".join(lines)
...
def gnn_summary(metrics: Mapping[str, Any]) -> str:
    ...
    for row in metrics["datasets"]:
        lines.append(f"| {row['dataset']} | {row['examples']} | {row['examples_with_ranked_entities']} | {row['examples_with_nonempty_path_context']} | {row['examples_with_mapped_evidence_units']} | {row['mean_ranked_entities']:.4f} | {row['mean_retrieved_paths']:.4f} |")
    lines.extend(["", "Exact ranked entities, relation-action traces, paths, and downstream reader context are in `per_example_retrieval.jsonl`.", ""])
    return "\n".join(lines)
...
def artifact_manifest(directory: Path, required_names: Sequence[str]) -> None:
    write_json(directory / "artifact_manifest.json", {
        ...
        "files": {
            name: {"size_bytes": (directory / name).stat().st_size, "sha256": sha256(directory / name)}
...
def common_run_state(args: argparse.Namespace, root: Path) -> dict[str, Any]:
    ...
    corpus_manifest = [
        {"path": path.relative_to(args.data_root).as_posix(), "size_bytes": path.stat().st_size, "sha256": sha256(path)}
        for path in corpus_files
    ]
    corpus_manifest_sha = canonical_hash(corpus_manifest)
    split_file_manifest = [row for row in corpus_manifest if row["path"].endswith("_subgraph_retrieval.jsonl")]
    ...
    return {
        ...
        "completed_eval_before": file_manifest(completed_eval_dir) if completed_eval_dir.exists() else None,
    }
...
def assert_protected_unchanged(args: argparse.Namespace, state: Mapping[str, Any]) -> None:
    ...
    if state["protected_before"] != file_manifest(state["protected_checkpoint_root"]):
        raise RuntimeError("Completed GNN/SAGE-QA checkpoint artifacts changed during baseline evaluation")
    ...
    if state["completed_eval_before"] != completed_after:
        raise RuntimeError("Completed GNN/SAGE-QA final TEST artifacts changed during baseline evaluation")
...
def base_lineage(
    ...
    return {
        ...
        "git_status_before": subprocess.check_output(["git", "status", "--short"], cwd=root, text=True).splitlines(),
        "python": platform.python_version(),
        "evaluation_command_argv": [sys.executable, str(Path(__file__).relative_to(root)), *sys.argv[1:]],
        "evaluation_command_display": subprocess.list2cmdline([sys.executable, str(Path(__file__).relative_to(root)), *sys.argv[1:]]),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/final_results/production_generator_d_v1_test_baselines"))
...
def main() -> None:
    ...
    args.data_root = (root / args.data_root).resolve() if not args.data_root.is_absolute() else args.data_root.resolve()
    ...
    if args.stage == "finalize":
        ...
        root_required = ("run_confirmation.json", "lexical_subgraph/artifact_manifest.json", "gnn_rag/artifact_manifest.json")
        write_json(output_dir / "artifact_manifest.json", {
            "self_excluded_from_hashes": True,
            "files": {name: {"size_bytes": (output_dir / name).stat().st_size, "sha256": sha256(output_dir / name)} for name in root_required},
        })
        print(json.dumps({"status": "complete", "lexical_examples": lexical_count, "gnn_rag_examples": gnn_count}), flush=True)
        return
    ...
    if (target_dir / "artifact_manifest.json").exists():
        ...
        print(json.dumps({"status": "already_complete", "stage": args.stage, "output_dir": str(target_dir)}), flush=True)
        return
    ...
    if args.stage == "lexical":
        ...
        for dataset, _, domain, gold_relative in DATASETS:
            ...
            golds = text_gold(gold_path, dataset, ids) if domain == "text" else ontology_gold(gold_path, ids)
            if set(golds) != ids:
        ...
        lineage = {
            **base_lineage(args, root, state, started_at, retrieval_frozen_at),
            ...
            "configuration": {"fixed_k": [1, 3], "adaptive_k": None, "scorer": "evaluation.eval_lexical_hotpot_subgraph.lexical_score", "compactness_bonus_weight": 0.02},
            "post_freeze_gold_fields": {"HotpotQA": ["supporting_facts"], "2WikiMultiHopQA": ["supporting_facts"], "ontology": ["gold explanations"]},
            "two_wiki_evidences_read": False,
            "test_candidate_inputs": {dataset: {"path": str(args.data_root / dataset / "test_subgraph_retrieval.jsonl"), "sha256": sha256(args.data_root / dataset / "test_subgraph_retrieval.jsonl"), **lexical_run_info[dataset]} for dataset, _, _, _ in DATASETS},
        ...
        (lexical_dir / "summary.md").write_text(lexical_summary(metrics), encoding="utf-8", newline="\n")
        write_json(lexical_dir / "lineage_metadata.json", lineage)
        artifact_manifest(lexical_dir, ("metrics.json", "per_example_retrieval.jsonl", "summary.md", "lineage_metadata.json"))
        print(json.dumps({"status": "complete", "stage": "lexical", "examples": len(all_records)}), flush=True)
        return
    ...
    for dataset, adapter_key, domain, _ in DATASETS:
        ...
        checkpoint_source = root / "third_party/GNN-RAG/gnn/checkpoint" / f"sageqa-{adapter_key}" / f"rearev_lstm_sageqa-{adapter_key}-final.ckpt"
        ...
        if marker_path.is_file():
            ...
            for name, expected_hash in marker["sha256"].items():
                if sha256(run_dir / name) != expected_hash:
                    raise ValueError(f"Completed GNN-RAG dataset artifact changed: {run_dir / name}")
            ...
            if run_dir.exists() and any(run_dir.iterdir()):
                raise FileExistsError(f"Incomplete GNN-RAG dataset requires inspection before replacement: {run_dir}")
            ...
            info_path, native_command = run_native_retriever(root, native_dir, checkpoint_source, run_dir, adapter_key)
            ...
            lineage = {
                ...
                "adapter": {**adapter_info, "native_answers_array_empty": True, "native_answer_field_present": False},
            ...
            dataset_files = ("data/test.json", str(info_path.relative_to(run_dir)).replace("\\", "/"), "frozen_retrieval.jsonl", "diagnostics.json", "lineage_metadata.json", "native_retriever.log")
            write_json(marker_path, {"status": "complete", "dataset": dataset, "sha256": {name: sha256(run_dir / name) for name in dataset_files}})
    ...
    lineage_doc = {
        **base_lineage(args, root, state, started_at, retrieval_frozen_at),
        ...
        "configuration": {"model": "ReaRev", "entity_dim": 50, "lm": "lstm", "num_iter": 2, "num_ins": 2, "num_gnn": 3, "relation_word_emb": False, "eps": 0.95, "text_max_candidates": args.text_max_candidates, "checkpoint_selection": "existing final checkpoint; no training"},
    ...
    artifact_manifest(gnn_dir, ("native_retrieval_diagnostics.json", "per_example_retrieval.jsonl", "summary.md", "lineage_metadata.json"))
    print(json.dumps({"status": "complete", "stage": "gnn", "examples": len(all_records)}), flush=True)
