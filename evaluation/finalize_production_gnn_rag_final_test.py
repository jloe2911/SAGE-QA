...
from data_processing.prepare_production_gnn_rag_clean import evidence_node, inference_sample, unit_node
...
def candidate_unit_maps(root: Path, data_root: Path, dataset: str, domain: str) -> dict[str, dict[str, str]]:
...
def main() -> None:
    ...
    output_dir = root / "outputs" / "final_results" / "production_generator_d_v1_test_baselines" / "gnn_rag"
    ...
    for dataset, cfg in DATASETS.items():
        ...
        for frozen in dataset_records:
            ...
            row["native_ranked_entity_support_overlap"] = best_overlap(predicted_units, references) if references else None
        ...
        gold_lineage[dataset] = {
            ...
            "fields": ["context", "supporting_facts"] if cfg["domain"] == "text" else ["gold explanations"],
        }
    ...
    write_json(output_dir / "artifact_manifest.json", {"self_excluded_from_hashes": True, "files": files}, exclusive=False)
    print(json.dumps({"status": "complete", "examples": len(evaluated), "prediction_freeze_sha256": freeze["prediction_freeze_sha256"]}))
