...
from data.build_subgraph_training_data import (  # noqa: E402
    build_split_map, generate_ontology_candidates, get_gold_explanations,
    is_connected as is_connected_units, parse_owl_context, unit_signature,
)
from data_processing.build_2wiki_subgraph_dataset import (  # noqa: E402
    generate_candidates as gen_2wiki, get_gold_support_units as gold_2wiki,
    normalize_2wiki_record,
)
from data_processing.build_hotpot_subgraph_dataset import (  # noqa: E402
    generate_candidates as gen_hotpot, get_gold_support_units as gold_hotpot,
    normalize_hotpot_record,
)
from data_processing.retrieval_contracts import (  # noqa: E402
    BUILDER_VERSION, clean_text_retrieval_input, git_provenance, sha256_file, write_json,
)
...
def gold_diag(candidates, golds, pool, context) -> dict[str, Any]:
    ...
    cs, gs, pool, context = [set(x) for x in candidates], [set(x) for x in golds if x], set(pool), set(context)
    union = set().union(*cs) if cs else set()
    recall = lambda have, gold: len(have & gold) / len(gold) if gold else 0.0
    preferred = max(
        gs,
        key=lambda g: (recall(context, g), recall(pool, g), max((recall(c, g) for c in cs), default=0), -len(g), tuple(sorted(g))),
        default=set(),
    )
    complete = any(g <= c for g in gs for c in cs)
    partial = any(g & c for g in gs for c in cs)
    return {
        "gold_explanation_count": len(gs), "preferred_gold": sorted(preferred),
        ...
        "complete_support_candidate": complete, "partial_support_any": partial,
...
def category(d: Mapping[str, Any], max_size: int) -> str | None:
    if d["complete_support_candidate"]: return None
    if not d["preferred_gold_in_context"]: return "F_context_missing_reference_support"
    if not d["preferred_gold_in_atomic_pool"]: return "A_required_evidence_missing_from_atomic_pool"
    if max_size > 0 and len(d["preferred_gold"]) > max_size: return "D_candidate_size_limit"
    if not d["preferred_gold_connected"]: return "E_connectivity_assumption"
    return "B_units_not_combined_or_C_pruned_by_budget"


def summarize(rows: list[dict], elapsed: float) -> dict[str, Any]:
    n, cc, ac = len(rows), [r["candidate_count"] for r in rows], [r["atomic_pool_size"] for r in rows]
    ...
    return {
        "examples": n, "zero_candidates": sum(x == 0 for x in cc),
        "zero_candidates_pct": 100 * sum(x == 0 for x in cc) / max(n, 1),
        "candidate_count": {"mean": statistics.fmean(cc) if cc else 0, "median": statistics.median(cc) if cc else 0, "distribution": hist(cc, [0, 32, 128, 320, 512, 1024])},
        "atomic_pool_size": {"mean": statistics.fmean(ac) if ac else 0, "median": statistics.median(ac) if ac else 0, "distribution": hist(ac, [0, 10, 20, 30, 40, 60, 80])},
        "candidate_size_distribution": dict(sorted(collections.Counter(sizes).items())),
        "pool_union_gold_recall": statistics.fmean(r["pool_union_gold_recall"] for r in rows) if rows else 0,
        ...
        "failure_categories": {k: {"count": v, "pct": 100*v/max(n, 1)} for k, v in sorted(fail.items())},
        "runtime_seconds": elapsed, "runtime_seconds_per_example": elapsed/max(n, 1),
    }
...
def audit_text(name: str, rows: list[dict], cfg: Mapping[str, int], seed: int):
    gen, gold_fn = (gen_2wiki, gold_2wiki) if name == "2WikiMultiHopQA" else (gen_hotpot, gold_hotpot)
    kg, details, start = KGConstructionConfig(backend="context_only", max_triples=64), [], time.perf_counter()
    for i, ex in enumerate(rows):
        g = gen(retrieval_example=clean_text_retrieval_input(ex), split_name="development_audit",
                max_sentences_per_example=cfg["atomic_budget"], max_subgraph_size=cfg["max_size"],
                max_candidates_per_question=cfg["candidate_cap"], kg_config=kg,
                llm_kg_constructor=None, kg_cache=None, kg_cache_path=None, kg_cache_lock=None, seed=seed+i)
        gold = gold_fn(ex, g["sent_lookup"]); context = list(g["sent_lookup"].values())
        d = {"example_id": g["example_id"], "question": g["question"],
             "atomic_pool_size": len(g["sentence_pool"]), "context_unit_count": len(context),
             "graph_context_unit_count": len(g["graph_context_units"]), "candidate_count": len(g["candidates"]),
             "candidate_sizes": [len(c) for c in g["candidates"]],
             "cross_document_candidate_count": sum(len({u.split("::",3)[1] for u in c}) > 1 for c in g["candidates"]),
             "both_supporting_sentences_in_atomic_pool": set(gold) <= set(g["sentence_pool"]),
             "evidences_absent": "evidences" not in ex, "kg_cache_used": False,
             **gold_diag(g["candidates"], [gold], g["sentence_pool"], context)}
        d["failure_category"] = category(d, cfg["max_size"]); details.append(d)
    s = summarize(details, time.perf_counter()-start)
    s.update({"source_file": str(TEXT[name].relative_to(ROOT)), "source_sha256": sha256_file(TEXT[name]),
              "sample_policy": f"deterministic seed-{seed} sample of official development",
              "config": dict(cfg),
              "both_supporting_sentences_in_atomic_pool_pct": 100*sum(d["both_supporting_sentences_in_atomic_pool"] for d in details)/max(len(details),1),
              "examples_with_cross_document_candidates_pct": 100*sum(d["cross_document_candidate_count"]>0 for d in details)/max(len(details),1),
              "evidences_absent_for_all_2wiki_rows": all(d["evidences_absent"] for d in details),
              "graph_context_context_only": True, "contaminated_cache_loaded": False})
    return s, details


def onto_rows(name: str):
    groups = json.loads(ONTO[name].read_text(encoding="utf-8")); split = build_split_map(groups, .65, .1)
    return [(gi, qi, item, qa) for gi, item in enumerate(groups) if split[gi] == "dev"
            for qi, qa in enumerate(item.get("QAs", [])) if get_gold_explanations(qa)]


def shape(s: str) -> str:
    u = " ".join(s.upper().split()); return ("SELECT" if u.startswith("SELECT") else "ASK" if u.startswith("ASK") else "OTHER") + f"_variables_{s.count('?')}"
...
def zero_trace(question, sparql, context, generated, cfg):
    ...
    for ax in context:
        ...
        if ae & ents or ap & props: direct.append(ax)
    if not ents and not props: reason, point = "query_entity_and_property_extraction_failure", "empty query signature before atomic filtering"
    elif not direct: reason, point = "candidate_unit_filtering", "no parsed context axiom overlaps query signature"
    else: reason, point = "beam_or_candidate_validation", "atomic pool exists but connected beam returned no candidates"
    return {"formal_query_shape": shape(sparql), "formal_query": sparql,
            "available_ontology_context_size": len(context), "query_entities": sorted(ents), "query_properties": sorted(props),
            "initial_directly_relevant_units": direct, "selected_atomic_units": generated["candidate_units"],
            "stages": {"parsed_context": len(context), "direct_relevance_filter": len(direct),
                       "atomic_budget_selection": len(generated["candidate_units"]), "connected_beam": len(generated["candidate_subgraphs"])},
            "budgets": dict(cfg), "root_cause": reason, "exact_disappearance_point": point}
...
def audit_onto(name: str, rows, cfg: Mapping[str, int]):
    ...
    for gi, qi, item, qa in rows:
        ...
        g = generate_ontology_candidates(question=q, sparql_query=sparql, owl_context=owl,
                max_subgraph_size=cfg["max_size"], min_subgraph_size=1, max_context_units=cfg["atomic_budget"],
                candidate_beam_width=cfg["beam_width"], max_candidate_subgraphs=cfg["candidate_cap"])
        d = {"example_id": f"{name}__g{gi}__q{qi}", "question": q, "formal_query_shape": shape(sparql),
             "atomic_pool_size": len(g["candidate_units"]), "context_unit_count": len(context),
             "candidate_count": len(g["candidate_subgraphs"]), "candidate_sizes": [len(c) for c in g["candidate_subgraphs"]],
             **gold_diag(g["candidate_subgraphs"], get_gold_explanations(qa), g["candidate_units"], context)}
        d["failure_category"] = category(d, cfg["max_size"]); details.append(d)
        if not g["candidate_subgraphs"]:
            zeros.append({"example_id": d["example_id"], "question": q, **zero_trace(q, sparql, context, g, cfg)})
    s = summarize(details, time.perf_counter()-start)
    s.update({"source_file": str(ONTO[name].relative_to(ROOT)), "source_sha256": sha256_file(ONTO[name]),
              "sample_policy": "all gold-labeled examples in deterministic group-stratified development split",
              "config": dict(cfg), "multiple_gold_explanation_semantics": "complete when any valid explanation is contained"})
    return s, details, zeros


def grid(kind: str, b: Mapping[str, int]):
    if kind == "text":
        return [dict(b), {**b,"atomic_budget":40}, {**b,"candidate_cap":1024}, {**b,"atomic_budget":40,"candidate_cap":1024}]
    return [
        dict(b),
        {**b, "max_size": 4},
        {**b, "max_size": 8},
...
def cid(cfg): return hashlib.sha256(json.dumps(dict(cfg),sort_keys=True).encode()).hexdigest()[:10]
...
def run_invariance_checks(output_dir: Path, seed: int):
    ...
    for name in TEXT:
        ...
        report["datasets"][name] = {
            "examples": len(rows),
            "compared": ["atomic_pool", "graph_context", "candidate_set", "candidate_pre_rank_order"],
...
def run(args):
    ...
    report = {"schema_version":"gold_free_candidate_audit_v2", "builder_version":BUILDER_VERSION,
              "development_only":True, "seed":args.seed, "text_sample_size":args.text_sample_size,
              "git":git_provenance(ROOT), "gold_available_during_candidate_generation":False,
              "gold_join_boundary":"post_hoc_diagnostics_only",
              "prohibited_actions":{"gnn_training":False,"test_evaluation":False,"adaptive_k":False,"answer_generation":False,"llm_or_api_calls":False},
              "datasets":{}, "budget_sensitivity":{}, "zero_candidate_traces":{}}
    for name in TEXT:
        rows, summaries, detail_sets = text_rows(name,args.text_sample_size,args.seed), [], []
        for i,cfg in enumerate(grid("text",TEXT_BASE[name]) if args.run_grid else [TEXT_BASE[name]]):
            s,d = audit_text(name,rows,cfg,args.seed); s["config_id"]=cid(cfg); summaries.append(s); detail_sets.append(d); jsonl(args.output_dir/name/f"details_{i}_{s['config_id']}.jsonl",d)
        if len(detail_sets) > 2:
            refine_budget_failures(detail_sets[0], detail_sets[2], summaries[0])
            jsonl(args.output_dir/name/f"details_0_{summaries[0]['config_id']}.jsonl",detail_sets[0])
    ...
    for name in (() if args.only_text else ONTO):
        ...
        for i,cfg in enumerate(grid("onto",ONTO_BASE) if args.run_grid else [ONTO_BASE]):
            ...
            s,d,z = audit_onto(name,evaluated_rows,cfg); s["config_id"]=cid(cfg); summaries.append(s); detail_sets.append(d); jsonl(args.output_dir/name/f"details_{i}_{s['config_id']}.jsonl",d)
            if i == 0: baseline_zeros = z
        ...
        report["datasets"][name], report["budget_sensitivity"][name], report["zero_candidate_traces"][name] = summaries[0], summaries, baseline_zeros
    write_json(args.output_dir/"audit_report.json",report); return report


def main():
    p=argparse.ArgumentParser(description=__doc__); p.add_argument("--text-sample-size",type=int,default=200); p.add_argument("--seed",type=int,default=42); p.add_argument("--run-grid",action="store_true")
    ...
    p.add_argument("--output-dir",type=Path,default=ROOT/"outputs/development_runs/gold_free_candidate_audit_v2")
    args = p.parse_args()
    if args.run_invariance_checks:
        print(json.dumps(run_invariance_checks(args.output_dir, args.seed), indent=2, ensure_ascii=False))
    elif args.refine_ontology_failures_from:
        print(json.dumps(refine_ontology_failures(args.refine_ontology_failures_from, args.output_dir), indent=2, ensure_ascii=False))
    else:
