...
def score_all_once(model, encoded, rows, device):
    with torch.no_grad():
        nodes = model.encode_graph(
            encoded["node_text_embeddings"],
            encoded["node_symbolic_features"],
            encoded["edge_index"],
        )
        pooled = torch.stack(
            [
                model.pool_subgraph(
                    nodes, torch.tensor(row["subgraph_node_ids"], dtype=torch.long, device=device)
                )
                for row in rows
            ]
        )
        symbolic = torch.tensor(
            [row["symbolic_features"] for row in rows], dtype=torch.float, device=device
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "outputs/diagnostics/production_generator_d_v1_causal_retrieval_diagnosis/causal_failure_taxonomy.json",
    )
    parser.add_argument(
        "--adaptive-policy",
        type=Path,
        default=ROOT / "outputs/development_runs/production_generator_d_v1_adaptive_k",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs/final_model_development/production_generator_d_static_hard_v1_dev",
    )
    ...
    policies = {
        domain: load_domain_policy(args.adaptive_policy, domain=domain)
    ...
    with ranking_path.open("x", encoding="utf-8") as handle:
        for dataset, data_dir, checkpoint_dir, domain, final_mode in DATASETS:
            ...
            for example_id, raw_rows in grouped_jsonl(
                args.data_root / data_dir / "dev_subgraph_retrieval.jsonl"
            ):
                ...
                with torch.no_grad():
                    encoded = encode_example_graph(
                        model, tokenizer, example, device, max_length=128
                    )
                ...
                if pair is not None and float(pair[1]["rank_target"]) > 0.0:
                    ...
                    ordering["positive_strictly_above_partial"] += int(
                ...
                adaptive = adaptive_v2_support_aggregate(
                    rankings["sageqa_final"], policy=policies[domain]
                )
                ...
                record = {
                    ...
                    "gnn_only": {
                        "metrics_k1": method_rows["gnn_only"][-1],
                        "top1_complete": bool(
                            rankings["gnn_only"][0].get("contains_any_gold_explanation")
                        ),
                        "top5": [
                            candidate_record(row, rank)
                            for rank, row in enumerate(rankings["gnn_only"][:5], 1)
                        ],
                    },
                    "sageqa_final": {
                        "metrics_k1": method_rows["sageqa_final"][-1],
                        "top1_complete": bool(
                            rankings["sageqa_final"][0].get("contains_any_gold_explanation")
                        ),
                        "top5": [
                            candidate_record(row, rank)
                            for rank, row in enumerate(rankings["sageqa_final"][:5], 1)
                        ],
                    },
                    "adaptive_sageqa": {
                        "selected_k": adaptive["selected_k"],
                        "metrics": adaptive_values,
                    },
                }
            ...
            per_dataset.append(
                {
                    ...
                    "complete_support_containment": {
                        key: value / examples for key, value in containment.items()
                    },
                    "complete_vs_hard_partial_ordering": {
                        **ordering,
                        "accuracy": ordering["positive_strictly_above_partial"]
                        / max(1, ordering["eligible_complete_vs_partial"]),
                    },
                }
            )
    baseline = {}
    ...
    diagnosed = {
        item["example_id"]
        for item in json.loads(args.taxonomy.read_text(encoding="utf-8"))["assignments"]
    ...
    for method in ("gnn_k1", "sageqa_k1", "adaptive_sageqa"):
        macro[method] = {
            metric: sum(row[method][metric] for row in per_dataset) / len(per_dataset)
            for metric in ("precision", "recall", "f1")
        }
    report = {
        "schema_version": "sageqa_static_hard_refinement_dev_v1",
        "split": "dev",
        "status": "complete",
        ...
        "diagnosed_443": {
            "expected": 443,
            "matched": len(diagnosed),
            "corrected_to_complete_sageqa_top1": corrected,
            "worsened_sageqa_k1_f1": worsened,
            "unchanged_sageqa_k1_f1": unchanged,
        },
    ...
    print(
        json.dumps({"status": "complete", "macro": macro, "diagnosed_443": report["diagnosed_443"]})
    )
