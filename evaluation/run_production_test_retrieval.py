...
DATASETS = (
    ("HotpotQA", "HotpotQA", "hotpotqa", "text", "sageqa_text_chain", "data/raw/hotpot_qa/distractor/validation-00000-of-00001.parquet"),
    ("2WikiMultiHopQA", "2WikiMultiHopQA", "2wiki", "text", "sageqa_text_chain", "data/raw/2WikiMultihopQA/data/validation-00000-of-00001.parquet"),
    ("FamilyOWL_1hop", "FamilyOWL_1hop", "familyowl_1hop", "ontology", "sageqa_proof", "data/raw/family/FamilyOWL_1hop.json"),
    ("FamilyOWL_2hop", "FamilyOWL_2hop", "familyowl_2hop", "ontology", "sageqa_proof", "data/raw/family/FamilyOWL_2hop.json"),
    ("pizza_100_1hop", "pizza_100_1hop", "pizza_100_1hop", "ontology", "sageqa_proof", "data/raw/pizza_100/pizza_100_1hop.json"),
    ("pizza_100_2hop", "pizza_100_2hop", "pizza_100_2hop", "ontology", "sageqa_proof", "data/raw/pizza_100/pizza_100_2hop.json"),
    ("pizza_250_1hop", "pizza_250_1hop", "pizza_250_1hop", "ontology", "sageqa_proof", "data/raw/pizza_250/pizza_250_1hop.json"),
    ("pizza_250_2hop", "pizza_250_2hop", "pizza_250_2hop", "ontology", "sageqa_proof", "data/raw/pizza_250/pizza_250_2hop.json"),
    ("OWL2Bench_1hop", "OWL2Bench_1hop", "OWL2Bench_1hop", "ontology", "sageqa_proof", "data/raw/owl2bench/OWL2Bench_1hop.json"),
    ("OWL2Bench_2hop", "OWL2Bench_2hop", "OWL2Bench_2hop", "ontology", "sageqa_proof", "data/raw/owl2bench/OWL2Bench_2hop.json"),
)
EXPECTED_THRESHOLDS = {
    "sageqa_final": {"text": 0.78, "ontology": 0.79},
    "gnn_only": {"text": 0.74, "ontology": 0.76},
}
PROHIBITED_TEST_FIELDS = {
    "gold_explanations", "gold_units", "gold_support_units", "raw_supporting_facts",
    "label", "rank_target", "best_set_f1_to_gold", "best_set_precision_to_gold",
    "best_set_recall_to_gold", "exact_match_any_gold", "contains_any_gold_explanation",
}
...
def grouped_jsonl(path: Path) -> Iterable[tuple[str, list[dict[str, Any]]]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if forbidden:
                raise ValueError(f"TEST candidate row contains gold-derived fields: {sorted(forbidden)}")
...
def verify_manifest(root: Path) -> dict[str, str]:
    ...
    return {"artifact_manifest.json": sha256(manifest_path), **{name: sha256(root / name) for name in recorded}}


def load_policy_bundle(root: Path, expected_method: str) -> tuple[dict[str, AdaptiveV2Policy], dict[str, Any], dict[str, str]]:
    ...
    configured_transitions = tuple((x["current_k"], x["continue_to_k"]) for x in config.get("transitions", []))
    if configured_transitions != TRANSITIONS:
        raise ValueError(f"Wrong adaptive transitions in {root}")
    expected = EXPECTED_THRESHOLDS[expected_method]
    if config.get("thresholds") != expected:
        raise ValueError(f"Wrong frozen thresholds in {root}: {config.get('thresholds')}")
    ...
    for domain, policy in policies.items():
        ...
        if policy.model.coef_[0].tolist() != described.get("coefficients_in_model_feature_order"):
            raise ValueError(f"Loaded model coefficients disagree with frozen config: {root} {domain}")
        if float(policy.model.intercept_[0]) != float(described.get("intercept")):
            raise ValueError(f"Loaded model intercept disagrees with frozen config: {root} {domain}")
...
def score_and_freeze(
    *, test_path: Path, checkpoint_path: Path, domain: str, final_mode: str,
    policies: Mapping[str, Mapping[str, AdaptiveV2Policy]], device: torch.device,
    candidate_batch_size: int, max_length: int,
    ...
    for example_index, (example_id, raw_rows) in enumerate(grouped_jsonl(test_path), start=1):
        ...
        with torch.no_grad():
            for start in range(0, len(example["candidate_rows"]), candidate_batch_size):
                ...
                probs = score_candidate_rows(model, encoded, batch, device)["probs"].detach().cpu().tolist()
        ...
        for method in ("gnn_only", "sageqa_final"):
            for setting in ("k1", "k3", "adaptive"):
                ...
                if setting == "adaptive":
                    ...
                    extras = f"{row['mean_selected_k']:.4f} | {row['median_selected_k']:.1f} | " + " | ".join(f"{dist[str(k)]['percentage']:.2f}%" for k in ALLOWED_K)
                else:
                    extras = "— | — | — | — | — | —"
                lines.append(f"| {dataset['dataset']} | {method} | {setting} | {row['precision']:.6f} | {row['recall']:.6f} | {row['f1']:.6f} | {extras} |")
    lines.extend(["", "## F1 comparisons", "", "| Dataset | SAGE k1 − GNN k1 | SAGE k3 − GNN k3 | SAGE adaptive − GNN adaptive | GNN adaptive − k1 | GNN adaptive − k3 | SAGE adaptive − k1 | SAGE adaptive − k3 |", "|---|---:|---:|---:|---:|---:|---:|---:|"])
    for dataset in metrics["datasets"]:
        d = dataset["f1_differences"]
        lines.append(f"| {dataset['dataset']} | {d['sageqa_k1_minus_gnn_k1']:+.6f} | {d['sageqa_k3_minus_gnn_k3']:+.6f} | {d['sageqa_adaptive_minus_gnn_adaptive']:+.6f} | {d['gnn_adaptive_minus_k1']:+.6f} | {d['gnn_adaptive_minus_k3']:+.6f} | {d['sageqa_adaptive_minus_k1']:+.6f} | {d['sageqa_adaptive_minus_k3']:+.6f} |")
    lines.extend(["", "## Frozen protocol confirmation", "", "Candidate generation was not rerun; GNNs were not retrained; adaptive policies were loaded from their saved scaler/model/threshold artifacts and were not refitted; Text-Chain and Proof reranking were unchanged; no TEST result altered any method or configuration; answer generation was not run.", ""])
    return "\n".join(lines)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/final_results/production_generator_d_v1_test_retrieval"))
...
def main() -> None:
    ...
    sage_policies, sage_config, sage_hashes = load_policy_bundle(args.sageqa_policy_dir, "sageqa_final")
    ...
    corpus_manifest = [{"path": str(path.relative_to(args.data_root)).replace("\\", "/"), "size_bytes": path.stat().st_size, "sha256": sha256(path)} for path in corpus_files]
    ...
    input_paths.extend(path for policy_dir in (args.sageqa_policy_dir, args.gnn_policy_dir) for path in policy_dir.iterdir() if path.is_file())
    ...
    for dataset, data_dir, checkpoint_dir, domain, final_mode, gold_relative in DATASETS:
        ...
        rows, run_info = score_and_freeze(
            test_path=test_path, checkpoint_path=checkpoint_path, domain=domain,
            final_mode=final_mode, policies=policies, device=device,
            candidate_batch_size=args.candidate_batch_size, max_length=args.max_length,
        )
        frozen[dataset] = rows
        dataset_lineage[dataset] = {
            "domain": domain, "test_candidate_path": str(test_path),
            "test_candidate_sha256": sha256(test_path), "checkpoint_path": str(checkpoint_path),
            "checkpoint_sha256": sha256(checkpoint_path), "gold_source_path": gold_relative,
            "gold_source_sha256": sha256(root / gold_relative), "sageqa_score_mode": final_mode,
            "gnn_score_mode": "neural", "inference_candidate_cap": 320, **run_info,
        }
    ...
    for dataset, _, _, domain, _, gold_relative in DATASETS:
        ...
        gold_path = root / gold_relative
        gold_by_id = text_gold(gold_path, dataset, ids) if domain == "text" else ontology_gold(gold_path, ids)
        if set(gold_by_id) != ids:
            missing = sorted(ids - set(gold_by_id))
            raise ValueError(f"Gold join incomplete for {dataset}: {missing[:3]}")
        setting_rows = {method: {setting: [] for setting in ("k1", "k3", "adaptive")} for method in ("gnn_only", "sageqa_final")}
        excluded = 0
        for prediction in predictions:
            ...
            if not eligible:
                excluded += 1
            record = {"dataset": dataset, "domain": domain, "split": "test", "example_id": prediction["example_id"], "evaluation_eligible": eligible, "exclusion_reason": None if eligible else "no_gold_support_explanation", "gold_explanations": golds, "methods": prediction["methods"]}
            for method in ("gnn_only", "sageqa_final"):
                for setting in ("k1", "k3", "adaptive"):
                    selected = record["methods"][method][setting]
                    scores = best_scores(selected["retrieved_evidence_units"], golds) if eligible else {"precision": None, "recall": None, "f1": None}
        ...
        methods = {method: {setting: summarize_setting(setting_rows[method][setting], adaptive=setting == "adaptive") for setting in ("k1", "k3", "adaptive")} for method in ("gnn_only", "sageqa_final")}
        ...
        metric_datasets.append({"dataset": dataset, "domain": domain, "prediction_examples": len(predictions), "evaluation_examples": evaluated, "excluded_no_gold_support": excluded, "methods": methods, "f1_differences": differences})

    hashes_after = {str(path): sha256(path) for path in input_paths}
    if hashes_before != hashes_after:
        raise RuntimeError("A frozen corpus/checkpoint/policy/reranker input changed during evaluation")
    metrics = {
        ...
        "split": "test", "metric_aggregation": "macro mean over support-bearing examples",
        "support_aggregation": "order-preserving deduplicated union of ranked candidate evidence units",
        "adaptive_allowed_k": list(ALLOWED_K), "datasets": metric_datasets,
    }
    completed_at = datetime.now(timezone.utc).isoformat()
    lineage = {
        "schema_version": "production_generator_d_v1_final_test_retrieval_lineage_v1",
        "status": metrics["status"], "started_from_code_commit": commit,
        "evaluator_sha256": sha256(evaluator_path), "git_status_before": status_before,
        ...
        "evaluation_command_argv": command, "evaluation_command_display": subprocess.list2cmdline(command),
        "configuration": {
            "data_root": str(args.data_root), "checkpoint_root": str(args.checkpoint_root),
            "sageqa_policy_dir": str(args.sageqa_policy_dir), "gnn_policy_dir": str(args.gnn_policy_dir),
            "output_dir": str(args.output_dir), "candidate_batch_size": args.candidate_batch_size,
            "max_length": args.max_length, "fixed_k": [1, 3], "adaptive_allowed_k": list(ALLOWED_K),
            "adaptive_transitions": [{"current_k": a, "continue_to_k": b, "feature_decision_rank": a} for a, b in TRANSITIONS],
            "sageqa_thresholds": sage_config["thresholds"], "gnn_thresholds": gnn_config["thresholds"],
            "size_penalty_argument": 0.01,
        },
        "generator_d_version": "generator_d_frozen_v1",
        "production_builder_version": "generator_d_production_v1",
        "corpus_manifest": corpus_manifest, "corpus_manifest_sha256": corpus_manifest_hash,
        "checkpoint_hashes": {dataset: values["checkpoint_sha256"] for dataset, values in dataset_lineage.items()},
        "adaptive_policy_artifact_hashes": {"sageqa_final": sage_hashes, "gnn_only": gnn_hashes},
        "reranker_implementation": {"path": str(reranker_path.relative_to(root)), "sha256": sha256(reranker_path), "text_score_mode": "sageqa_text_chain", "ontology_score_mode": "sageqa_proof"},
        "datasets": dataset_lineage,
        "gold_boundary": {"all_predictions_and_selected_supports_frozen_before_any_gold_source_opened": True, "predictions_frozen_at_utc": predictions_frozen_at, "gold_access_started_after_predictions_frozen": True, "two_wiki_columns_read_post_freeze": ["id", "context", "supporting_facts"], "two_wiki_evidences_read": False, "two_wiki_answer_read": False},
        "frozen_protocol_confirmations": {"candidate_generation_rerun": False, "gnns_retrained": False, "adaptive_policies_refitted": False, "adaptive_thresholds_changed": False, "text_chain_changed": False, "proof_reranking_changed": False, "reranker_weights_unchanged": True, "test_result_changed_method_or_configuration": False, "answer_generation_run": False, "manuscript_updated": False, "commit_or_push_performed": False, "all_frozen_inputs_hash_unchanged_during_run": True},
    ...
    write_json(args.output_dir / "artifact_manifest.json", {"schema_version": "production_generator_d_v1_final_test_retrieval_manifest_v1", "status": metrics["status"], "self_excluded_from_hashes": True, "files": {path.name: {"size_bytes": path.stat().st_size, "sha256": sha256(path)} for path in files}})
    print(json.dumps({"output_dir": str(args.output_dir), "complete": True, "datasets": len(metric_datasets)}))
